...
- BaseEntity / SoftDeletableEntity 상속으로 공통 필드가 중복 없이 관리되는지
...
4. Like와 OrderItem은 `BaseEntity`만 상속합니다. `SoftDeletableEntity`를 상속하지 않으므로 `deletedAt` 컬럼 자체가 없습니다. Like는 좋아요 취소 시 레코드를 삭제(Hard Delete)하고, OrderItem은 삭제 없이 영구 보존합니다.
